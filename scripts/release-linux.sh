#!/usr/bin/env bash
# Build Linux desktop app locally, deploy to server, then trigger CI version bump + full release.
#
# Usage:
#   ./scripts/release-linux.sh                  # patch bump (default)
#   ./scripts/release-linux.sh minor            # minor bump
#   ./scripts/release-linux.sh major            # major bump
#
...
echo "╔══════════════════════════════════════════════════╗"
echo "║  Manthra Linux Release — bump: $BUMP_TYPE$(printf '%*s' $((19 - ${#BUMP_TYPE})) '')║"
echo "╚══════════════════════════════════════════════════╝"
echo ""

# ── Step 1: Build Linux AppImages ─────────────────────────────────────────────
echo "▶  Step 1/3 — Building Linux desktop app (x64 + arm64)..."
echo ""
...
echo "▶  Step 2/3 — Deploying Linux builds to ${VM_HOST}..."
echo ""
...
find "$DESKTOP_DIR" \( -name "*.AppImage" -o -name "*.yml" \) | sort | while read -r f; do
